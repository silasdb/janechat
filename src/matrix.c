...
void matrix_request_file(FileInfo fileinfo) {
	...
	Str *url = str_new(.cstr = "/_matrix/media/v3/download/");
...
static void process_direct_event(const char *sender, json_t *roomid) {
	...
	event.roominfo.id = str_new(.cstr = json_string_value(roomid));
	event.roominfo.sender = str_new(.cstr = sender);
...
static void process_room_event(json_t *item, const char *roomid) {
	...
	if (streq(json_string_value(type), "m.room.name")) {
		...
		MatrixEvent event;
		event.type = EVENT_ROOM_INFO;
		event.roominfo.id = str_new(.cstr = roomid);
		event.roominfo.name = str_new(.cstr = name);
		...
		event.roomcreate.id = str_new(.cstr = roomid);
		event_handler_callback(event);
		...
		event.roomjoin.roomid = str_new(.cstr = roomid);
		event.roomjoin.senderid =
			str_new(.cstr = json_string_value(sender));
		...
		if (name) {
			...
			event.roomjoin.sendername =
				str_new(.cstr = json_string_value(name));
		} else
...
static void process_timeline_event(json_t *item, const char *roomid) {
	...
	if (streq(json_string_value(type), "m.room.message")) {
		...
		event.msg.roomid = str_new(.cstr = roomid);
		event.msg.msg.sender = str_new(.cstr = json_string_value(sender));
		...
		|| streq(json_string_value(msgtype), "m.file")) {
			...
			if (!m)
				m = "(unknown mimetype)";
			event.msg.msg.fileinfo.mimetype = str_new(.cstr = m);
			...
			event.msg.msg.fileinfo.uri = str_new(.cstr = 
		...
		if (streq(json_string_value(msgtype), "m.text")) {
			event.msg.msg.text.content
				= str_new(.cstr = json_string_value(body));
		} else {
		...
		event.msg.roomid = str_new(.cstr = roomid);
		event.msg.msg.sender = str_new(.cstr = json_string_value(sender));
		event.msg.msg.text.content = str_new(.cstr = "== encrypted message ==");
		event_handler_callback(event);
		str_decref(event.msg.roomid);
		str_decref(event.msg.msg.sender);
		str_decref(event.msg.msg.text.content);
...
static void process_error(json_t *root) {
	...
	event.error.errorcode = str_new(.cstr = json_string_value(json_object_get(root, "errcode")));
	event.error.error = str_new(.cstr = json_string_value(json_object_get(root, "error")));
	event_handler_callback(event);
...
static void process_push_rules(json_t *rule) {
	...
	json_array_foreach(actions, i, item) {
		...
		if (streq(json_string_value(item), "dont_notify")) {
			...
			event.roomnotifystatus.roomid = str_new(.cstr = roomid);
